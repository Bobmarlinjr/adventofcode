...
for list_pair in Day1_input.split("\n"):
    list_pair = list_pair.split("   ")
    left_list.append(int(list_pair[0]))
    right_list.append(int(list_pair[1]))
...
for i in range(len(left_list_sorted)):
    differences.append(abs(left_list_sorted[i] - right_list_sorted[i]))
